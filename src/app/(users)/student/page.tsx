        ...
        const fetchDashboardData = async () => {
            ...
            try {
                ...
                if (axios.isAxiosError(err)) {
                    ...
                    // Jika bukan error dari axios, tampilkan pesan umum
    ...
    const getIcon = (iconName: string) => {
        switch (iconName) {
            case "ClipboardListIcon":
                return <ClipboardListIcon className="h-5 w-5 text-primary" />;
            case "BookmarkIcon":
                return <BookmarkIcon className="h-5 w-5 text-primary" />;
            case "LightbulbIcon":
        ...
        return (
            <div className="flex items-center justify-center min-h-screen p-4 bg-white">
                <div className="bg-red-50 border-l-4 border-red-500 text-red-700 p-4 rounded-lg shadow-sm max-w-md w-full">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => window.location.reload()} className="mt-2 text-red-700 hover:bg-red-100 rounded-full text-sm">
                        Coba Lagi
                    </Button>
                </div>
            </div>
            ...
            <div className="max-w-7xl mx-auto space-y-8 ">
                    ...
                    <CardContent className="p-6">
                        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                            <div className="space-y-3">
                                ...
                                <p className="text-gray-300 text-lg max-w-2xl">Halo Siswa Lihat perkembangan gaya belajarmu untuk belajar lebih efektif.</p>
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                        ...
                        <Card className="bg-white shadow-lg rounded-lg border border-gray-200">
                            <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-gray-200">
                                <CardTitle className="text-lg font-semibold text-primary">Status Tes Terakhir</CardTitle>
                                <CalendarIcon className="h-5 w-5 text-primary" />
                            </CardHeader>
                            <CardContent className="pt-4">
                                <p className="text-2xl font-bold text-primary">{test_data.lastTestDate}</p>
                                <p className="text-sm text-gray-600 mt-1">Terakhir mengikuti tes</p>
                                <div className="mt-4 flex items-center">
                                    <Badge className={`px-3 py-1 rounded-full text-sm ${test_data.testCompleted ? "bg-white text-primary border border-primary/50" : "bg-gray-50 text-gray-600 border border-gray-200"}`}>
                                        {test_data.testCompleted ? "Selesai" : "Belum Selesai"}
                                    </Badge>
                                    <span className="ml-2 text-sm text-gray-600">{test_data.totalTests} tes dilakukan</span>
                                </div>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="bg-white shadow-lg rounded-lg border border-gray-200">
                            <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-gray-200">
                                <CardTitle className="text-lg font-semibold text-primary">Gaya Belajar</CardTitle>
                            ...
                            <CardContent className="pt-4 space-y-3">
                                {Object.entries(test_data.results).map(([dimension, { type, level }]) => (
                                    <div key={dimension} className="flex justify-between items-center">
                                        ...
                                        <Tooltip>
                                            <TooltipTrigger>
                                                <Badge className={`px-3 py-1 rounded-full text-sm ${getBadgeColor(level)}`}>{level}</Badge>
                                            </TooltipTrigger>
                                            <TooltipContent>
                                                <p className="text-sm">
                        ...
                        <Card className="bg-white shadow-lg rounded-lg border border-gray-200 lg:col-span-3">
                            <CardHeader className="flex flex-row items-center justify-between pb-3 border-b border-gray-200">
                                <CardTitle className="text-lg font-semibold text-primary">Detail Gaya Belajar</CardTitle>
                                        ...
                                        return (
                                            <div key={dimension} className="space-y-3">
                                                <div className="flex justify-between items-center">
                                                    <span className="text-sm font-medium text-gray-700 capitalize">{dimensionMaps[dimension as keyof typeof dimensionMaps]?.title}</span>
                                                    <Tooltip>
                                                        <TooltipTrigger>
                                                            <Badge className={`px-3 py-1 rounded-full text-sm ${getBadgeColor(level)}`}>{level}</Badge>
                                                        </TooltipTrigger>
                        ...
                        <Card className="bg-white shadow-lg rounded-lg border border-gray-200 lg:col-span-3">
                            <CardHeader className="pb-3 border-b border-gray-200">
                                <CardTitle className="text-lg font-semibold text-primary">Teknik Belajar untuk Kamu</CardTitle>
                            </CardHeader>
                            <CardContent className="pt-4">
                                <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                                    {learning_tips.map((tip, index) => (
                                        <div key={index} className="rounded-lg bg-white p-4 hover:bg-white transition-all duration-300">
                                            <div className="mb-3 flex items-center">
                                                {getIcon(tip.icon)}
                                                <h3 className="ml-2 text-sm font-medium text-primary">{tip.title}</h3>
                                            </div>
                                            <p className="text-sm text-gray-700">{tip.description}</p>
                                            <Button
                                                variant="ghost"
                                                onClick={() => handleOpenDialog(tip)}
                                                className="mt-3 text-sm font-medium text-primary hover:bg-primary/20 rounded-full px-4 py-2"
                    ...
                    <div className="space-y-6">
                        ...
                        <Card className="bg-white shadow-lg rounded-lg border border-gray-200">
                            <CardContent className="flex flex-col items-center gap-6 py-8">
                                <BarChart3Icon className="h-20 w-20 text-gray-200 animate-pulse" />
                                <div className="text-center space-y-3">
                                    <h2 className="text-2xl font-bold text-primary">Belum Ada Tes Gaya Belajar</h2>
                                ...
                                <Button onClick={() => (window.location.href = "/student/question")} className="bg-primary hover:bg-primary text-white text-lg font-medium px-8 py-4 rounded-full" aria-label="Mulai tes gaya belajar">
                                    Mulai Tes Sekarang
                                </Button>
                            </CardContent>
                        </Card>
                        ...
                        <Card className="bg-white shadow-lg rounded-lg border border-gray-200">
                            ...
                            <CardContent className="pt-4">
                                <p className="text-gray-600 text-lg">Lengkapi tes untuk melihat gaya belajar Anda di sini.</p>
                            </CardContent>
                        </Card>
                ...
                <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    <DialogContent className="bg-white rounded-lg p-6 max-w-md">
                        <DialogHeader>
                            <DialogTitle className="text-lg font-semibold text-primary">{selectedTip?.title || "Detail Teknik Belajar"}</DialogTitle>
                            <DialogDescription id="dialog-description" className="text-sm text-gray-700">
                                {selectedTip?.detailedDescription || "Tidak ada penjelasan detail tersedia."}
                            </DialogDescription>
                        </DialogHeader>
                        <DialogClose asChild>
                            <Button className="mt-4 bg-primary hover:bg-primary text-white text-sm font-medium px-6 py-3 rounded-full" aria-label="Tutup dialog teknik belajar">
                                Tutup
                            </Button>
                        </DialogClose>
                    </DialogContent>
